...
for nodes in ${nodeCounts[@]}; do   # 32 nodes equals 128 GPUs
    ...
    echo startSpark --time=04:00:00 --nodes=$((nodes+1)) --partition=gpu2 --gres=gpu:$gpusPerNode --cpus-per-task=$coresPerNode
    for (( workPerNode=3*2**36; workPerNode<=3*2**41; workPerNode*=2 )); do
    {
        ...
        for nSlices in $((nodes*gpusPerNode-gpusPerNode/2)) $((nodes*gpusPerNode)); do
            echo sparkSubmit ~/scaromare/MontePi/multiNode/multiGpu/scala/MontePi.jar \
